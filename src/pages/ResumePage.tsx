...
import {
  buildJDResumePrompt,
  buildMaterialReorganizePrompt,
} from "@/prompts/resumeGen";
import { jdsRepository } from "@/storage";
import { resumeDraftRepository } from "@/storage/repository";
import { JDRecord, Material, ResumeDraft, ResumeGenerationMode } from "@/types";
import { createId, formatDate } from "@/utils/id";
import { clearEditCache, readEditCache, writeEditCache } from "@/utils/editCache";

const RESUME_EDIT_CACHE = "aidesk:resume-edit-cache";
...
function draftTitle(mode: ResumeGenerationMode, jd?: JDRecord) {
  if (mode === "jd") return `${jd?.title ?? "目标岗位"} 简历`;
  return `未命名简历 ${new Date().toLocaleDateString("zh-CN")}`;
}

export function ResumePage() {
  const { materials: allMaterials, save: saveMaterial } = useMaterials();
  const materials = allMaterials;
  const personalMaterials = useMemo(
    () => materials.filter((material) => (material.type ?? "personal") === "personal"),
    [materials],
  );
  const [jds, setJds] = useState<JDRecord[]>([]);
  const [drafts, setDrafts] = useState<ResumeDraft[]>([]);
  const [selectedDraftId, setSelectedDraftId] = useState("");
  const [mode, setMode] = useState<ResumeGenerationMode>("jd");
  ...
  useEffect(() => {
    void Promise.all([jdsRepository.getAll(), resumeDraftRepository.getDrafts()]).then(([storedJDs, storedDrafts]) => {
      setJds(storedJDs);
      setDrafts(storedDrafts);
      if (storedJDs[0]) setJdId(storedJDs[0].id);
      const cached = readEditCache<{ title: string; content: string; mode: ResumeGenerationMode; jdId: string; materialIds: string[] }>(RESUME_EDIT_CACHE);
      if (cached) { setTitle(cached.title); setContent(cached.content); setMode(cached.mode); setJdId(cached.jdId); setSelectedMaterialIds(cached.materialIds); }
    });
  }, []);

  useEffect(() => { if (!content.trim() && !title.trim()) return; const timer = window.setTimeout(() => writeEditCache(RESUME_EDIT_CACHE, { title, content, mode, jdId, materialIds: selectedMaterialIds }), 2000); return () => window.clearTimeout(timer); }, [content, title, mode, jdId, selectedMaterialIds]);

  const selectedMaterials = personalMaterials.filter((material) =>
    selectedMaterialIds.includes(material.id),
  );
  useEffect(() => {
    const available = new Set(personalMaterials.map((material) => material.id));
  ...
  const toggleMaterial = (materialId: string) =>
    setSelectedMaterialIds((current) => {
      if (!personalMaterials.some((material) => material.id === materialId)) return current;
  ...
  const loadDraft = (draft: ResumeDraft) => {
    ...
    clearEditCache(RESUME_EDIT_CACHE);
  ...
  const generate = async () => {
    if (!selectedMaterials.length) {
      setToast("请至少勾选一条素材后再生成");
      return;
    }
    if (mode === "jd" && !selectedJD) {
      setToast("JD 定制版需要先选择一条历史 JD");
      return;
    }
    setLoading(true);
    setContent("");
    ...
    const prompt =
      mode === "jd"
        ...
        : buildMaterialReorganizePrompt(materialPayload(selectedMaterials));
    try {
  ...
  const saveDraft = async (contentOverride = content, silent = false) => {
    if (!contentOverride.trim()) {
      if (!silent) setToast("暂无可保存的简历内容");
      return;
    }
    const now = new Date().toISOString();
    const draft: ResumeDraft = {
      id: selectedDraftId || createId(),
      title: title.trim() || draftTitle(mode, selectedJD),
      mode,
      ...
      updatedAt: now,
      targetJdId: mode === "jd" ? jdId : null,
      createTime:
        drafts.find((item) => item.id === selectedDraftId)?.createTime ??
        Date.now(),
      updateTime: Date.now(),
    };
    await resumeDraftRepository.saveDraft(draft);
    setDrafts(await resumeDraftRepository.getDrafts());
    setSelectedDraftId(draft.id);
    setTitle(draft.title);
    clearEditCache(RESUME_EDIT_CACHE);
    if (!silent) setToast("简历草稿已保存");
  };

  const renameDraft = async (titleValue: string) => {
    if (!renameDraftId) return;
    await resumeDraftRepository.renameDraft(renameDraftId, titleValue);
    setDrafts(await resumeDraftRepository.getDrafts());
  ...
  const deleteDraft = async () => {
    ...
    await resumeDraftRepository.deleteDraft(deleteDraftId);
    setDrafts(await resumeDraftRepository.getDrafts());
  ...
  return (
    <div>
      <header>
        <h1 className="text-3xl font-semibold">简历生成</h1>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          所有内容均来自您勾选的真实经历，AI 仅优化表达与结构，不新增经历
        </p>
      </header>
        ...
        <aside className="space-y-5">
          <section className="panel p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                className={`rounded-xl border px-3 py-2.5 text-sm ${mode === "jd" ? "border-indigo-500 bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300" : "dark:border-slate-700"}`}
                onClick={() => setMode("jd")}
              >
                <span className="block">JD 定制版</span>
                <span className="mt-1 block text-[11px] font-normal text-slate-400">
                  根据目标岗位要求，整理优化已选素材。
                </span>
              </button>
              <button
                className={`rounded-xl border px-3 py-2.5 text-sm ${mode === "materials" ? "border-indigo-500 bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300" : "dark:border-slate-700"}`}
                onClick={() => setMode("materials")}
              >
                <span className="block">通用基础版</span>
                <span className="mt-1 block text-[11px] font-normal text-slate-400">
                  基于已选素材，按标准结构生成简历。
                </span>
              </button>
            ...
            <label className="mt-4 block text-sm font-medium">
              ...
              <input
                ...
                placeholder={draftTitle(mode, selectedJD)}
              />
            </label>
            <div className="mt-5 flex items-center justify-between">
              <h3 className="text-sm font-semibold">选择素材</h3>
              <div className="flex items-center gap-2">
            ...
            <p className="mt-2 text-xs text-slate-400">
              仅个人真实经历可用于简历生成，外部参考资料已置灰
            </p>
            <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
              {materials.length ? (
                materials.map((material) => (
                  <label
                    className={`flex gap-3 rounded-xl border p-3 transition dark:border-slate-800 ${material.type === "reference" ? "cursor-not-allowed opacity-50" : "cursor-pointer hover:border-indigo-300"}`}
                    key={material.id}
                  >
                    <input
                      className="mt-1 accent-indigo-600"
                      type="checkbox"
                      disabled={material.type === "reference"}
            ...
            <p className="mt-4 text-xs leading-5 text-slate-400">
              AI 只能使用勾选素材。没有素材支撑的部分会标记为素材缺口或待量化。
            </p>
            <button
              className="button-primary mt-5 w-full"
              title={
                mode === "jd" && !selectedJD ? "请先选择目标 JD" : undefined
              }
              disabled={loading || (mode === "jd" && !selectedJD)}
              onClick={generate}
            >
              {loading ? (
                <>
          ...
          <section className="panel p-5">
            ...
            <div className="mt-4 space-y-2">
              {drafts.length ? (
                ...
                <EmptyState icon={FileText} title="还没有生成过简历" description="基于你的素材库，快速生成针对性的简历内容" actionLabel="去生成简历" onAction={() => document.querySelector<HTMLButtonElement>('button[aria-label="生成简历"]')?.focus()} />
              )}
            </div>
          </section>
        </aside>
